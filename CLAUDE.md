...
| build-error-resolver     | Gradle ビルドエラー・ktlint・AndroidLint の最小差分修正                                      |
...
| android-performance-reviewer | メモリリーク・ANR・バッテリー消費・過剰な再コンポジション・オーバードロー  |
| android-architecture-reviewer| MVVM + Clean Architecture 準拠・レイヤー分離・依存方向                     |
| kotlin-reviewer              | Kotlin 慣用句・Coroutines/Flow パターン・Null Safety・ktlint 準拠          |
| compose-ui-reviewer          | Jetpack Compose パターン・状態ホイスティング・副作用処理・アクセシビリティ |
...
| Skill                           | Purpose                                                                               |
| ------------------------------- | ------------------------------------------------------------------------------------- |
| kotlin-best-practices           | Kotlin 慣用句・Null Safety・拡張関数・データクラス・スコープ関数・sealed class        |
| android-mvvm-clean-architecture | MVVM + Clean Architecture・UI/Domain/Data 層分離・依存方向・UseCase/Repository パターン |
| jetpack-compose-patterns        | Compose UI・状態ホイスティング・副作用（LaunchedEffect 等）・パフォーマンス最適化    |
| android-coroutines-flow         | Coroutines・Flow/StateFlow/SharedFlow・ViewModel Scope・構造化並行性                 |
| android-di-hilt                 | Dagger/Hilt による DI・モジュール定義・スコープ管理・テスト用モック注入              |
| android-testing                 | JUnit4/5・Mockito/MockK・Espresso・Compose UI Test・TDD サイクル                     |
| android-navigation              | Navigation Component・SafeArgs・ディープリンク・バックスタック管理                   |
| room-database                   | Room DAO/Entity・TypeConverter・マイグレーション戦略・クエリ最適化                   |
| retrofit-api-patterns           | Retrofit/OkHttp・Interceptor・エラーハンドリング・Result ラッパー                    |
| security-patterns               | Android セキュリティ・EncryptedSharedPreferences・証明書ピニング・ProGuard 設定       |
...
| detect-debug-logging     | Write後 | `.kt` 内の `Log.v`/`Log.d` を警告（`Log.e`/`Log.w`/`Log.i` は許可）                          |
| require-tmux-for-builds  | Bash前  | `./gradlew assembleDebug` 等の長時間ビルドを tmux 外で実行するのをブロック                    |
...
| ドメイン判定 | 適用スキル名 |
|---|---|
| `.kt` 全般 | `test-driven-development`, `verification-before-completion`, `iterative-retrieval`, `kotlin-best-practices` |
| UI 層（`presentation/`, `ui/`, `@Composable`） | 上記 + `jetpack-compose-patterns`, `android-mvvm-clean-architecture` |
| Domain 層（`domain/`, `usecase/`, `UseCase`） | 上記 + `android-mvvm-clean-architecture`, `android-coroutines-flow` |
| Data 層（`data/`, `repository/`, `datasource/`） | 上記 + `android-mvvm-clean-architecture` |
| Room DB（`*Dao.kt`, `*Entity.kt`, `*Database.kt`） | 基本 + `room-database` |
| Retrofit（`*Api.kt`, `*Service.kt`, `network/`） | 基本 + `retrofit-api-patterns` |
| DI（`*Module.kt`, `hilt/`, `dagger/`） | 基本 + `android-di-hilt` |
| Navigation（`*NavGraph.kt`, `NavHost`） | 基本 + `android-navigation` |
| テスト（`*Test.kt`, `androidTest/`） | `android-testing`, `test-driven-development` |
| セキュリティ関連 | `security-patterns` |
| アーキテクチャ設計 | `architecture-patterns`, `android-mvvm-clean-architecture` |
...
- 検証コマンド実行（`./gradlew test`, `./gradlew lint`, `git diff --stat`）
...
- Kotlin + ktlint 準拠（インデント4スペース、命名規則：クラス PascalCase、変数/関数 camelCase）
- 既存のコード規約・パターンを踏襲
- コードやコメントにエモジを入れない
...
- `./gradlew lint` と `./gradlew test` をコミット前に実行
