...
description: "複数レビュアーの結果を統合し、重複排除・矛盾解決・優先度調整・カバレッジマトリクスを生成する（React Native版）"
model: opus
...
- 最も具体的な修正案を持つ指摘を残す
- 統合した旨を注記する（例: 「TYPE-001 と ARCH-002 を統合」）
...
各指摘には以下の項目を必ず含めること:
- **レビュアー**: 指摘元のレビュアー名
- **カテゴリ**: 指摘の分類（Security / TypeScript / Architecture / UI/RN / Performance / Testing）
- **指摘内容**: 問題の詳細な説明
...
## 矛盾検出
### [CONFLICT-001] [矛盾の概要]
- **レビュアーA**: [rn-architecture-reviewer] -- [指摘内容と根拠]
- **レビュアーB**: [rn-performance-reviewer] -- [指摘内容と根拠]
- **ユーザー判断が必要**: [判断すべきポイント]
...
| REQ-002: Happy Path | - | - | ARCH-001 | UI-003 | PERF-001 | - | Covered |
