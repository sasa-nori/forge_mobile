...
1. **stack-docs-researcher** -- Context7 MCP経由で関連フレームワーク（React Native, TypeScript, TanStack Query, React Navigation, Zustand/Redux Toolkit 等）の公式ドキュメントから該当機能のベストプラクティスを取得
...
### Task 1: [タスク名]（推定: X分）
- **対象ファイル**: `src/features/xxx/hooks/useXxx.ts`（新規 or 既存）
- **やること**: [具体的な変更内容]
- **検証方法**: [テストコマンド（例: npx jest src/features/xxx）]
