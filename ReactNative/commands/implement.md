...
2. **implementer 完了後の検証**
   - `npx jest` / `npx tsc --noEmit` / `git diff --stat`
...
- [ドメイン固有スキル名（例: typescript-best-practices, rn-ui-patterns, react-query-patterns）]
...
- npx jest でテストがパスすること
- npx tsc --noEmit で型チェックがパスすること
