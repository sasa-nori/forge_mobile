...
- 最も具体的な修正案を持つ指摘を残す
- 統合した旨を注記する（例: 「DART-001 と ARCH-002 を統合」）
...
各指摘には以下の項目を必ず含めること:
- **レビュアー**: 指摘元のレビュアー名（例: security-sentinel, flutter-performance-reviewer）
- **カテゴリ**: 指摘の分類（Security / Performance / Architecture / Dart / BLoC-Riverpod / Test）
- **指摘内容**: 問題の詳細な説明
...
#### 指摘 1: [タイトル（例: flutter_secure_storage を使用していない）]
...
#### 指摘 2: [タイトル]
- **レビュアー**: flutter-performance-reviewer
- **優先度**: High
- **確信度**: MEDIUM
- **対象ファイル**: `ファイルパス:行番号`
- **指摘内容**: [問題の詳細な説明]
...
## 矛盾検出
### [CONFLICT-001] [矛盾の概要]
- **レビュアーA**: [flutter-architecture-reviewer] -- [指摘内容と根拠]
- **レビュアーB**: [flutter-performance-reviewer] -- [指摘内容と根拠]
- **ユーザー判断が必要**: [判断すべきポイント]
...
| 仕様項目 | Security | Performance | Architecture | Dart | BLoC/Riverpod | Test | カバー状態 |
|---|---|---|---|---|---|---|---|
| REQ-001: Happy Path | - | PERF-001 | - | DART-002 | - | - | Covered |
| REQ-001: Error Scenario | SEC-003 | - | - | - | BLOC-001 | TEST-001 | Covered |
| REQ-002: Happy Path | - | - | ARCH-001 | - | BLOC-002 | - | Covered |
