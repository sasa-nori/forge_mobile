...
1. **stack-docs-researcher** -- Context7 MCP経由で関連フレームワーク（Flutter, Dart, BLoC, Riverpod, Drift, Dio 等）の公式ドキュメントから該当機能のベストプラクティスを取得
...
2. **複数の有効なアーキテクチャ**: BLoC vs Riverpod など技術的に同等な設計アプローチが複数存在し、プロジェクトの方針として選択が必要な場合
3. **セキュリティ設計判断**: 認証方式、flutter_secure_storage の暗号化戦略、アクセス制御モデルなど、セキュリティに関わる設計判断が必要な場合
...
### Task 1: [タスク名]（推定: X分）
- **対象ファイル**: `lib/features/xxx/presentation/xxx_page.dart`（新規 or 既存）
- **やること**: [具体的な変更内容]
- **検証方法**: [テストコマンド: flutter test / flutter analyze]
