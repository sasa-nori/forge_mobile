...
1. **security-sentinel**: flutter_secure_storage・シークレット検出・認証・認可・ネットワーク通信・難読化
2. **dart-reviewer**: Dart 慣用句・Null Safety・`!` 演算子・`late`・extension・sealed class・dart format 準拠
3. **flutter-architecture-reviewer**: Clean Architecture 準拠・BLoC/Riverpod レイヤー分離・依存方向・パッケージ構成
4. **bloc-riverpod-reviewer**: BLoC/Cubit イベント・ステート設計・BlocProvider スコープ・Riverpod Provider 設計・副作用処理
5. **flutter-performance-reviewer**: const Widget 活用・不要な setState・ListView.builder・画像キャッシュ・メモリ管理
6. **flutter-test-reviewer**: flutter_test カバレッジ・Widget テスト品質・BLoC テスト・mocktail モック設計
...
### [カテゴリ: Security / Performance / Architecture / Dart / BLoC-Riverpod / Test]
...
2. 同一パターンの却下が **2 回以上**蓄積した場合:
   ...
   - 例: 「flutter-architecture-reviewer が Riverpod の直接 Repository アクセスを常に指摘するが、このプロジェクトでは設計上許容している」
     → flutter-architecture-reviewer のチェックリストにプロジェクト固有の例外を追加提案
...
- ノイズ候補（低確信度）: X件
- 矛盾検出: X件
- 起動レビュアー: [security-sentinel, dart-reviewer, ...]

## P1: クリティカル
### [SECURITY-001] flutter_secure_storage を使用していない
- **重要度**: P1
- **確信度**: HIGH
- **ファイル**: `lib/data/local/user_preferences.dart:42`
- **問題**: 機密情報が SharedPreferences に平文保存されている
- **修正案**: flutter_secure_storage を使用してデータを暗号化する
...
| 仕様項目 | Security | Dart | Architecture | BLoC/Riverpod | Performance | Test | カバー状態 |
|---|---|---|---|---|---|---|---|
| REQ-001: Happy Path | - | DART-001 | - | BLOC-002 | - | - | Covered |
