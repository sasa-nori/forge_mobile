...
2. **実装ファイル（`.dart` 等）を Read してはならない**
...
2. **implementer 完了後の検証**
   - `flutter test` / `flutter analyze` / `git diff --stat`
...
全タスク完了後:
1. `flutter test` で全テスト実行
2. `flutter analyze` で静的解析
3. `dart format --output=none --set-exit-if-changed .` でフォーマット確認
4. `git diff --stat` で変更ファイル一覧を確認
5. プロジェクト/グローバル同期確認: 変更ファイルに `~/.claude/` 配下のファイルが含まれる場合、リポジトリ内に対応ファイル（`agents/`, `commands/`, `reference/` 等）が存在するか確認し、不一致があればコピーによる同期を実施する
...
- [ドメイン固有スキル名（dart-best-practices, flutter-bloc-clean-architecture, flutter-ui-patterns 等）]
...
- flutter test がパスすること
- flutter analyze がパスすること
