...
2. **実装ファイル（`.swift` 等）を Read してはならない**
...
2. **implementer 完了後の検証**
   - `swift test` / `swiftlint lint` / `git diff --stat`
...
全タスク完了後:
1. `swift test` または `xcodebuild test -scheme [Scheme] -destination 'platform=iOS Simulator,name=iPhone 16'` で全テスト実行
2. `swiftlint lint` でLintチェック
...
- [ドメイン固有スキル名]
  例: swift-best-practices, swiftui-patterns, swift-concurrency,
      ios-mvvm-clean-architecture, ios-testing, ios-networking,
      coredata-swiftdata, ios-dependency-injection, security-patterns
...
- テストがパスすること（swift test または xcodebuild test）
- SwiftLint チェックがパスすること（swiftlint lint）
