...
1. **stack-docs-researcher** -- Context7 MCP経由で関連フレームワーク（Swift, SwiftUI, Combine, Xcode, SPM等）の公式ドキュメントから該当機能のベストプラクティスを取得
...
1. **リサーチ結果の矛盾**: 複数のリサーチエージェントが矛盾する推奨を返した場合
2. **複数の有効なアーキテクチャ**: 技術的に同等な設計アプローチが複数存在し、プロジェクトの方針として選択が必要な場合
3. **セキュリティ設計判断**: 認証方式、Keychain設計、暗号化戦略、アクセス制御モデルなど、セキュリティに関わる設計判断が必要な場合
...
- ユニットテスト: [対象と方針]
- 統合テスト: [対象と方針]
- UIテスト（XCUITest）: [対象と方針]

## タスク

### Task 1: [タスク名]（推定: X分）
- **対象ファイル**: `Sources/Feature/XXXView.swift`（新規 or 既存）
- **やること**: [具体的な変更内容]
- **検証方法**: `swift test` または `xcodebuild test -scheme [Scheme]`
