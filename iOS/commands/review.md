...
| `.swift` ファイルが含まれる | **swift-reviewer** |
| `*View.swift` / `Views/` / SwiftUI コード | **swiftui-reviewer**, **ios-architecture-reviewer** |
| Data 層（`*Repository.swift`, `*DataSource.swift`, `data/`） | **ios-performance-reviewer** |
| `*Tests.swift` / `UITests/` | **ios-test-reviewer** |
...
1. **security-sentinel**: Keychain・ATS・SSL証明書ピニング・Deeplink検証・PII保護・シークレット漏洩
2. **ios-performance-reviewer**: メモリリーク・retain cycle・メインスレッドブロッキング・バッテリー消費・過剰再描画
3. **ios-architecture-reviewer**: MVVM + Clean Architecture 準拠・レイヤー分離・依存方向・UseCase/Repository パターン
4. **swift-reviewer**: Swift 慣用句・force unwrap・weak self・Swift Concurrency・SwiftLint 準拠
5. **swiftui-reviewer**: SwiftUI パターン・状態ホイスティング・副作用処理・アクセシビリティ・ナビゲーション
6. **ios-test-reviewer**: XCTest/XCUITest カバレッジ・async テスト・モック設計・テスト品質
...
2. **矛盾解決**: 相反する指摘をフラグ
3. **優先度調整**: 確信度に基づく横断的再評価。LOW 確信度の Medium/Low はノイズ候補として分離
...
### [カテゴリ: Security / Performance / Architecture / Swift / SwiftUI / Test]
...
Critical 指摘あり
  ...
  └── アーキテクチャ変更不要
      ...
      → 再レビューで新たな Critical が出た場合はユーザーに報告（無限ループ防止）
```
...
Critical/High 指摘に基づく修正（自動修正またはユーザー指示による修正）が実施された場合、Main Agent が review-summary.md の「修正内容」セクションに修正内容を追記する。各修正について以下を記録する:
- **対応**: 修正済み / スコープ外 / 次回対応
- **変更内容**: 何をどう修正したか
- **変更ファイル**: 修正したファイルパス
...
2. 同一パターンの却下が **2 回以上**蓄積した場合:
   ...
   - 例: 「ios-architecture-reviewer が特定パターンを常に指摘するが、このプロジェクトでは設計上許容している」
     → ios-architecture-reviewer のチェックリストにプロジェクト固有の例外を追加提案

3. ノイズ候補（LOW 確信度 High/Medium）が却下された場合:
...
- Low（ノイズ候補・低確信度）: X件
- 矛盾検出: X件
- 起動レビュアー: [security-sentinel, swift-reviewer, ...]

## Critical: クリティカル
### [SEC-001] Keychain を使わずに UserDefaults に認証トークンを保存
- **重要度**: Critical
- **確信度**: HIGH
- **ファイル**: `Sources/Auth/AuthRepository.swift:42`
- **問題**: 認証トークンが UserDefaults に平文で保存されている
- **修正案**: Keychain API（Security フレームワーク）を使用して保存する
